...
class UserRepository implements Observer
{
    ...
    {
        $this->dispatcher = (require __DIR__ . '/events.php')();
        $this->dispatcher->attach($this, "users:deleted");
    }
    ...
    public function createUser(array $data, bool $silent = false): User
    {
        ...
        if (!$silent) {
            $this->dispatcher->trigger("users:created", $this, $user);
        }
    ...
    public function updateUser(User $user, array $data, bool $silent = false): ?User
    {
        ...
        if (!$silent) {
            $this->dispatcher->trigger("users:updated", $this, $user);
        }
    ...
    public function deleteUser(User $user, bool $silent = false): void
    {
        ...
        if (!$silent) {
            $this->dispatcher->trigger("users:deleted", $this, $user);
        }
    }
}
